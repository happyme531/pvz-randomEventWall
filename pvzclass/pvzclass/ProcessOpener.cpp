...
LPCWSTR ProcessOpener::WindowTitle = TEXT((LPCWSTR)"Plants vs. Zombies");
LPCWSTR ProcessOpener::Directory = TEXT((LPCWSTR)".");
LPCWSTR ProcessOpener::ExecuteableName = TEXT((LPCWSTR)"PlantsVsZombies.exe");
...
DWORD ProcessOpener::OpenByProcessName(LPCWSTR processName)
{
	...
	while (ret)
	{
		if (!lstrcmp(pe32.szExeFile, (LPCSTR)processName))pid = pe32.th32ProcessID;
